...
def fixture_rows(name: str) -> list[dict[str, str]]:
    return list(csv.DictReader(io.StringIO(fixture_text(name))))


def fixture_header(name: str) -> list[str]:
    return next(csv.reader(io.StringIO(fixture_text(name))))
...
def recorded_names() -> list[str]:
    ...
    return sorted(p.name for p in FIXTURES.glob("*.csv"))

...
`swiss-efv-mcp` is published to [PyPI](https://pypi.org/project/swiss-efv-mcp/)
so it can be installed with `uvx swiss-efv-mcp` / `pip install swiss-efv-mcp`.
...
2. Under **Add a new pending publisher**, fill in:
   - **PyPI Project Name:** `swiss-efv-mcp`
   - **Owner:** `malkreide`
   - **Repository name:** `swiss-efv-mcp`
...
- Confirm the new version at <https://pypi.org/project/swiss-efv-mcp/>.

### 7. Smoke-test the published package

```bash
uvx swiss-efv-mcp@latest    # confirm it starts over stdio
```
...
pip install --index-url https://test.pypi.org/simple/ --no-deps swiss-efv-mcp
...
- [ ] New version visible on PyPI and `uvx swiss-efv-mcp` works
